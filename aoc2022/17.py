...
with open('17.txt') as finput: content = finput.read().strip()
...
def print_chamber(rock=pd.Series()):
    ...
    for y in range(top, 1):
        print("".join(
            c.get((x,y), "@" if complex(x,y) in rock.values else ".") for x in range(7))
        )
    print()
...
for i in range(2022):
    ...
    while True:
        ...
        if push == "<":
            pushed_rock = rock -1
            if pushed_rock.apply(np.real).min() >= 0 and not pushed_rock.isin(chamber).any():
                rock = pushed_rock
        elif push == ">":
            pushed_rock = rock + 1
            if pushed_rock.apply(np.real).max() < 7 and not pushed_rock.isin(chamber).any():
                rock = pushed_rock
