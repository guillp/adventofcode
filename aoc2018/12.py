...
def solve(content: str, nb_generations: int = 20) -> int:
    initial_state, spreads = content.split("\n\n")
    rules = {tuple(c == '#' for c in llcrr): x == "#" for spread in spreads.splitlines() for llcrr, x in
             [spread.split(" => ")]}
    ...
    for generation in range(nb_generations):
        state = frozenset(i for i in range(min(state) - 5, max(state) + 6) if
                          rules.get(tuple(i + j in state for j in (-2, -1, 0, 1, 2))))
...
with open("12.txt") as f:
    content = f.read()
print(solve(content))
print(solve(content, 50000000000))
